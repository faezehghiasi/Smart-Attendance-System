...
// Read a student ID from EEPROM based on the index
...
// Get the number of students stored in EEPROM
...
// Set the number of students stored in EEPROM
...
// Get an array of all student IDs from EEPROM
...
// Delete a student ID from EEPROM
        ...
        if (current_student == studentCode) {
            ...
            if (num_of_students == 1) {
                ...
                for (uint16_t j = i; j < num_of_students - 1; j++) {
                    ...
                    _delay_ms(4);  // Delay to ensure write completion
                }
            ...
            num_of_students--;
            SET_NUM_OF_STUDENTS(num_of_students);
            _delay_ms(4);  // Delay to ensure update completion
            break;
        }
    }
...
// Write a student ID to a specific index in EEPROM
